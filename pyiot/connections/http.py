...
class HttpConnection:
    def __init__(
        ...
        if self.user and self.password:
            self._headers[
                "Authorization"
            ] = f"Basic {b64encode(f'{self.user}:{self.password}'.encode('utf-8')).decode('ascii')}"
    ...
    def post(
        ...
        return self.request(
            path, method="POST", data=data, raw=raw, headers=headers, query=query
        )
    ...
    def request(
        ...
        req = Request(
            url=f"{self.url}/{quote(path)}{_query}",
            method=method,
            data=_data.encode("utf8"),
            headers=headers,
        )
        try:
