...
class BraviaApi:
    ...
    def check_power_status(self) -> str:
        ...
        try:
            self.conn.timeout = 2
            ret: Response = self.conn.post(
                path="system", data=self._cmd("getPowerStatus")
            )
            if ret.code == 200:
                result = ret.json
                if "result" in result:
    ...
    def off(self):
        """Power off tv"""
        self.conn.post(
            path="system", data=self._cmd("setPowerStatus", params=[{"status": False}])
        )
    ...
    def set_mute(self, status: bool):
        ...
        Args:
            status (bool)"""
        self.conn.post(
            path="audio", data=self._cmd("setAudioMute", params=[{"status": status}])
        )
    ...
    def set_volume(self, value: int, target: str = ""):
        self.conn.post(
            ...
            data=self._cmd(
                "setAudioVolume",
                params=[{"volume": f"{value}", "target": target}],
                pid=666,
            ),
        )
    ...
    def set_power_saving_mode(self, mode: str):
        ...
        self.conn.post(
            path="system",
            data=self._cmd("setPowerSavingMode", params=[{"mode": f"{mode}"}], pid=52),
        )
    ...
    @staticmethod
    def _cmd(
        cmd: str, params: List[Any] = [], pid: int = 10, version: str = "1.0"
    ) -> Dict[str, Any]:
