...
class Clock(BaseDevice):
    ...
    def get_sun_info(self) -> None:
        ...
        with request.urlopen(
            "https://api.sunrise-sunset.org/json?lat=52.2319581&lng=21.0067249&formatted=0"
        ) as r:
            try:
                ...
                if "results" in ret:
                    ret = ret["results"]
                    sunrise_utc = (
                        datetime.fromisoformat(ret.get("sunrise", "")) + utcoffset
                    )
                    sunset_utc = (
                        datetime.fromisoformat(ret.get("sunset", "")) + utcoffset
                    )
